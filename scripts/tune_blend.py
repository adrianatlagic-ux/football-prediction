"""Empirically find the classifier/Poisson blend weight that maximizes accuracy."""
import sys
...
from src.data_loader import load_wm2026_relevant, load_completed_matches
from src.feature_engineering import build_features, get_feature_columns, encode_result
from src.poisson_model import predict_scorelines
from src.predictor import FootballPredictor

CLASSES = ["H", "D", "A"]

# Replicate train()'s data loading + split
df_raw = load_wm2026_relevant(since_year=1990)
df_raw["result"] = df_raw.apply(lambda r: encode_result(r["home_goals"], r["away_goals"]), axis=1)
history = load_completed_matches()
history = history[history["date"].dt.year >= 1990].reset_index(drop=True)
...
df_test = df_raw.iloc[split:].reset_index(drop=True)

# Load the trained model (already trained without class_weight=balanced)
p = FootballPredictor(model_path=Path("model.joblib"))
p._history = history
p._feature_cols = feature_cols

# Classifier probabilities for the whole test set
X = test[feature_cols]
clf_proba = p.model.predict_proba(X)  # (n, 3) in H,D,A order

# Poisson probabilities per test match
...
for i, row in df_test.iterrows():
    try:
        ...
        poi_proba[i] = clf_proba[i]  # fallback
